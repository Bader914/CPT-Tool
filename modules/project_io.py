...
def render_project_io():
    ...
    Staat bovenaan stap 1: een eerdere analyse zet ook de uitgangspunten terug,
    dus je moet hem kunnen openen vóórdat je die zelf invult.
    """
    # Wordt normaal in stap 2 gezet; vanuit stap 1 bestaat de sleutel nog niet.
    if "sonderingen" not in st.session_state:
        st.session_state.sonderingen = {}

    n = len(st.session_state.sonderingen)
    ...
    with st.expander("💾 Analyse opslaan / openen", expanded=(n == 0)):
        ...
        with col_a:
            ...
            if n == 0:
                ...
                st.download_button(
                    ...
                    data=project_naar_json(project),
        ...
        with col_b:
            ...
            if up is not None:
                try:
                    project = lees_project(up.read().decode("utf-8"))
                    hersteld = herstel_sonderingen(project)
